...
(function(){
  ...
  textMatches.forEach(function(m){
    ...
    if (best && groups.pallet.indexOf(best) !== -1) return;
  ...
  groups.pillar.forEach(function(p){ if (p._group) svg.appendChild(p._group); });
  ...
  function productOptions(){
    var entries = [];
    groups.shelf.forEach(function(p){ gridOf(p, stock.shelves[p.id]).cells.forEach(function(cell){ entries = entries.concat(cellItems(cell)); }); });
    palletUnits.forEach(function(p){ entries = entries.concat(palletItems(p)); });
    Object.values(productSources).forEach(function(data){ (data.lots || []).forEach(function(lot){ entries.push({name:lot.n,code:lot.c}); }); });
    return WarehouseProducts.catalog(entries);
  }   // id -> the doc as the live snapshot last saw it
  var shelfIndex = {};
  ...
  function makeRow(name, qty, threshold, expiry, lot){
    ...
    nameInput.type = 'hidden'; nameInput.value = name || '';
    var picker = WarehouseProducts.create(nameInput, productOptions);
  ...
  function readRows(){
    return Array.prototype.slice.call(itemRowsEl.children).map(function(row){
      var inputs = row.querySelectorAll(':scope > input');
      return {
  ...
  function openCell(p, index){
    ...
    var first = itemRowsEl.querySelector('.product-trigger');
  ...
  function cellButton(p, g, i){
    ...
    var b = h('button', 'cell');
    b.type = 'button';
    b.style.gridRow = row; b.style.gridColumn = col;
    var pos = h('span', 'pos', row + '단 ' + col + '칸' + (items.length > 1 ? ' · ' + items.length + '건' : ''));
    b.appendChild(pos);
    if (items.length){
  ...
  function shelfCard(p){
    ...
    grid.style.gridTemplateColumns = 'repeat(' + g.cols + ', minmax(220px, 1fr))';
    grid.style.gridTemplateRows = 'repeat(2, auto)';
    if (g.cols > 2) card.classList.add('wide-card');
  ...
  function clearPrivateView(){
    ...
    stock={shelves:{},pallets:{}}; productSources={}; active=null;
  ...
  InventorySecurity.start(firebase.app(),function(identity){
    ...
    if(myTier<1)return;
    ['p3l','ecount','carton'].forEach(function(id){
      subscriptions.push(db.doc('stock/'+id).onSnapshot(function(doc){
        if(generation===identityGeneration) productSources[id]=doc.exists?doc.data():{};
      },function(){ if(generation===identityGeneration) delete productSources[id]; }));
    });
